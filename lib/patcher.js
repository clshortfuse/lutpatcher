...
export function patchDDS(dds) {
  ...
  for (const point of luts.points) {
    ...
    if (scaleDown || scaleUp || scaledBlackY || scaledWhiteY !== 1) {
      ...
      const reduceRed = linearNormalization(linearX, 0, 1, 1 / (1 - levels.minChannel), 1);
      const increaseRed = linearNormalization(linearX, 0, 1, 1, 1 / levels.maxChannel);
      const scaledRed = (1 - ((1 - red) * reduceRed)) * increaseRed;
      const reduceGreen = linearNormalization(linearY, 0, 1, 1 / (1 - levels.minChannel), 1);
      const increaseGreen = linearNormalization(linearY, 0, 1, 1, 1 / levels.maxChannel);
      const scaledGreen = (1 - ((1 - green) * reduceGreen)) * increaseGreen;
      const reduceBlue = linearNormalization(linearZ, 0, 1, 1 / (1 - levels.minChannel), 1);
      const increaseBlue = linearNormalization(linearZ, 0, 1, 1, 1 / levels.maxChannel);
      ...
      // Linear scaling
      const decreaseY = linearNormalization(blackDistance, 0, totalRange, 1 / (1 - scaledBlackY), 1);
      const increaseY = linearNormalization(whiteDistance, 0, totalRange, 1 / scaledWhiteY, 1);
      ...
      if (newRed !== (red) || newGreen !== green || newBlue !== blue) {
        ...
        if (crushing || clipping) {
          ...
          console.warn(
            ...
            y.toFixed(2),
            z.toFixed(2),
            `rgb(${[red, green, blue].map(asRGB8).join(',')})`,
            '=>',
            `rgb(${[scaledRed, scaledGreen, scaledBlue].map(asRGB8).join(',')})`,
            '=>',
            'x',
            targetY / currentY,
            '=>',
            `rgb(${[newRed, newGreen, newBlue].map(asRGB8).join(',')})`,
