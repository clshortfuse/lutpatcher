...
export function patchDDS(dds) {
  ...
  for (const point of luts.points) {
    ...
    if (scaleDown || scaleUp || blackY || whiteY !== 1) {
      ...
      const currentY = yFromLinearRGB(red, green, blue);
      ...
      if (linearX === 1 && linearY === 1 && linearZ === 1) {
        ...
        const scaledRed = Math.max(0, (1 - ((1 - red) * (reduceRed))) * (increaseRed));
        const scaledGreen = Math.max(0, (1 - ((1 - green) * (reduceGreen))) * (increaseGreen));
        const scaledBlue = Math.max(0, (1 - ((1 - blue) * (reduceBlue))) * (increaseBlue));

        const scaledY = yFromLinearRGB(scaledRed, scaledGreen, scaledBlue);

        
        const increaseY = linearNormalization(whiteDistance, 0, totalRange, 1 / levels.whiteY, 1);

        const increased = scaledY * increaseY;
        targetY = increased;
        ...
        if (targetY < blackY) {
          const useShadowBoost = true;
          if (useShadowBoost) {
            if (targetY < 0) {
              targetY = 0;
              // TODO: Find new value when crushing
              // May reference neutral but could ruin gradients. Crush for now
            }
            const shadowCompensationPercentage = 1 - blackY;
            targetY *= linearNormalization(targetY, 0, blackY, 1 + shadowCompensationPercentage, 1);
          } else {
            // shadowOverride
            // TODO: Iterate on concept
            const lowestRelativeY = (1 - ((1 / (1 - blackY))));
            targetY = linearNormalization(targetY, lowestRelativeY, blackY, 0, blackY);
          }
        }
        ...
        if (targetY <= 1) {
          ...
          // Could also clip to white
          // Could also pick brightest hue-matching value (use xyY?)
        }
      }
      // Debug points
      if (targetY < 0) {
      ...
      if (x === 0 && y === 0 && z === 0) {
        const isBlack = true;
        if (currentY !== blackY) {
          if (Math.abs(currentY - blackY) >= Number.EPSILON * 6) {
            const isWrong = true;
          } else {
        ...
        const isWhite = true;
        if (currentY !== whiteY) {
          if (Math.abs(currentY - whiteY) >= Number.EPSILON * 6) {
            const isWrong = true;
          } else {
            const isFloatingPointError = true;
          }
